def scan_dir(root_dir, dir = '', &block)
  files = []
  Dir.entries(File.join(root_dir, dir)).each do |entry|
    ...
    if File.directory?(file_path)
      files += scan_dir(root_dir, local_path) do |path|
        block.call(path)
      end
    else
      files << block.call(file_path)
    end
  end
  files
end

def scan_dir_hash(root_dir)
  scan_dir(root_dir) do |path|
    result = nil
    File.open(path, "r") do |file|
      result = { digest: file.read, path: path }
    end
    result
  end
end
